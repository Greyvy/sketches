...
let sketch = ({ gl, width, height }) => {
    ...
    let torus_render = regl({
        ...
        void main() {
            ...
            vec4 color = vec4(0.0);
            color += texture2D(u_texture0, uv)*0.1964825501511404;
            color += texture2D(u_texture0, uv+(off1/u_resolution))*0.2969069646728344;
            color += texture2D(u_texture0, uv-(off1/u_resolution))*0.2969069646728344;
            color += texture2D(u_texture0, uv+(off2/u_resolution))*0.09447039785044732;
            color += texture2D(u_texture0, uv-(off2/u_resolution))*0.09447039785044732;
            color += texture2D(u_texture0, uv+(off3/u_resolution))*0.010381362401148057;
            color += texture2D(u_texture0, uv-(off3/u_resolution))*0.010381362401148057;
            ...
            */
            u_matrix: (stats, props) => {
                ...
                let tra = mat4.translate([], mat4.identity([]), [
                    map(u_random[0], 0, 1, -0.75, 0.75),
                    map(u_random[1], 0, 1, -0.75, 0.75),
                    map(u_random[2], 0, 1, -0.75, 0.75)
                ])
    ...
    let torus_elements = new Array(128)
        .fill({})
