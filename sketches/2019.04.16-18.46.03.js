...
let sketch = ({ gl, width, height }) => {
    ...
    return ({ playhead }) => {
        ...
        let i = 0
        for (let y = 0; y < noise_array.length; ++y) {
            for (let x = 0; x < noise_array[y].length; ++x) {
                let nx = map(x, 0, 127, 0, 1)
                let ny = map(y, 0, 127, 0, 1)
                let nz = map(i++, 0, 128*128, 0, 1)
                noise_array[y][x][0] = map(simplex.noise2D(Math.sin(playhead*PI), nx), -1, 1, 0, 255)
                noise_array[y][x][1] = map(simplex.noise3D(Math.sin(playhead*PI), nx, ny), -1, 1, 0, 255)
                noise_array[y][x][2] = map(simplex.noise4D(Math.sin(playhead*PI), nx, ny, nz), -1, 1, 0, 255)
                noise_array[y][x][3] = 0
            }
        }
