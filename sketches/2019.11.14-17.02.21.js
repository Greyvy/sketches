...
let sketch = async ({ gl, width, height }) => {
    ...
    let line_draw = regl({
        ...
        attribute vec2 a_position, a_normal, a_velocity;
        ...
        void main() {
            ...
            vec4 fft = texture2D(u_fft, vec2(0.0, (1.0+position.y)/2.0));
            ...
            // vec2 contrast = vec2(0.493055, 0.1601061);
            vec2 contrast = fft.xy*0.125; // u_spot.xy;
            float line_width = 0.0125+(abs(dot(contrast*0.5, position.xy)));
        ...
        uniforms: {
            ...
            u_matrix: (stats, props) => {
                ...
                return mat4.rotate([], translate, TAU, [0, 0, 1.0-u_random[2]*2.0])
            },
    ...
    let player = await load_sound(`/assets/2019_11_15.mp3`)
    ...
    for (let i = 0; i < N_LINES; ++i) {
        ...
        for (let j = 0; j <= N_POINTS; ++j) {
            ...
            let nx = simplex.noise3D(jt, it, Math.cos(it+0))
            let ny = simplex.noise3D(jt, it, Math.sin(it+0))
    ...
    let damp_fft_values = new Array(lines_geo.length).fill(0)
        ...
        render({ playhead, frame }) {
            ...
            line_target1024.use(() => {
                for (let i = 0; i < lines_geo.length; ++i) {
                    let index = Math.floor((i/lines_geo.length)*fft_r.length)
                    damp_fft_values[i] = lerp(damp_fft_values[i], fft_r[index], 0.15)
                    ...
                    for (let j = 0; j <= N_POINTS; ++j) {
                        ...
                        let s = 1
                        let r = 6
                        let nx = simplex.noise4D(jt*s, it*s,
                            Math.cos(it+playhead*TAU)*r, Math.sin(i+playhead*TAU)*r)
                        let ny = simplex.noise3D(jt*s, it*s, Math.sin(i+playhead*TAU)*r)

                        let x = (1.0-it*2.0)+(nx*(t*0.24))
                        let y = (1.0-jt*2.0)+(ny*(t*0.24))
                        line_geo.push(x, y)
                    }
                    lines_geo[i] = line_make(line_geo)
