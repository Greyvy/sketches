...
let sketch = async ({ gl, width, height }) => {
    ...
    let torus = torus_make({ majorSegments: 16*8, minorSegments: 8*8, arc: Math.PI*2.0 })
    let text = text_make()
    ...
    let torus_draw = regl({
        ...
        void main() {
            ...
            position.z += (length(fft)*0.25)*(ease(sin(t), 1.5)*(1.0-(u_index/u_length)));
    ...
    let player = await load_sound(`/assets/2019_08_23.mp3`)
    ...
    return {
        ...
        render({ playhead }) {
            ...
            for (let i = 0; i < fft_v.length; ++i) {
                fft_r[i] = Math.floor(map(fft_v[i], -90, 0, 0, 255))
            }
