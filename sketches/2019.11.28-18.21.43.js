...
let sketch = async ({ gl, width, height }) => {
    ...
    function lights_update(playhead) {
        ...
        for (let i = 0; i < N_LIGHTS; ++i) {
            ...
            lights[`light${i}_specular`] = lights_data[i].specular
        }
        return lights
    }
    ...
    let diamond_draw = regl({
        ...
        void main() {
            ...
            position.xyz *= 1.0+length(fft)*0.125;
            position.z += 1.0+length(fft)*0.25;
        ...
        uniforms: Object.assign({
            u_model: function(stats, {u_time, u_random, u_translate}) {
                ...
                result = mat4.scale([], result, [0.125, 0.25, 0.125])
    ...
    let player = await load_sound(`/assets/2019_11_22.mp3`)
    ...
    let STRIDE = 4
    let diamonds = new Array(STRIDE*STRIDE)
        .fill(0)
        .map(function(v, i, a) {
            let x = 1.0-((i%STRIDE)/(STRIDE-1))*2.0
            let y = 1.0-((Math.floor(i/STRIDE))/(STRIDE-1))*2.0
            return {
                u_random: [rand(), rand(), rand()],
                u_resolution: [width, height],
                u_background: [...bgc, 1],
                u_translate: [x, y, 0.125]
            }
        })
    ...
    return {
        ...
        render({ playhead, frame }) {
            ...
            render_target1024.use(function() {
                ...
                diamond_draw(diamonds.map(function(diamond) {
                    diamond.u_time = playhead
                    s = Object.assign(diamond, lights)
                    return s
                }))
            })
