...
let sketch = async ({ gl, width, height }) => {
    ...
    let torus_draw = regl({
        ...
        uniforms: Object.assign({
            u_model: function(stats, {u_time, u_random, u_translate}) {
                ...
                let scale = 0.125*u_random[0]

                result = mat4.translate([], result, u_translate)
                result = mat4.rotate([], result, random[0]+random[1]+u_time*TAU, u_random)
    ...
    let torusus = new Array(NUM_PER_ROW*NUM_ROWS)
        .fill(0)
        .map(function(v, i, a) {
            let t = i/(a.length-1)
            let x = 1.0-((i%NUM_PER_ROW)/(NUM_PER_ROW-1))*2.0
            let y = 1.0-(Math.floor(i/NUM_PER_ROW)/(NUM_ROWS-1))*2.0
            return {
