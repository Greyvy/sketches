...
let sketch = async ({ gl, width, height }) => {
    ...
    let text_make = function(colour) {
        ...
        ctx.fillText('MIERROR', canvas.width/2, canvas.height/2+(fs/2))
    ...
    let bgc = hsluv.hsluvToRgb([rand()*360, rand()*100, rand()*20])

    let icosphere_colour = hsluv.hsluvToRgb([rand()*360, 40+rand()*20, 40+rand()*20])
    ...
    let icosphere_draw = regl({
        ...
        void main() {
            ...
            vec2 uv = v_uv;
            vec2 uv_t = vec2(uv.x, mix(st.y, uv.y, st.y+sin(u_time*PI)));
            vec2 uv_s = uv;
            vec4 fft = texture2D(u_fft, uv);

            // uv_t.x += 1.0-pow(abs((1.0-(u_time*2.0))), 1.0);
            uv_t.x += 1.0-pow(abs((1.0-(u_random.x*2.0))), 1.0);
            uv_t.y += u_time;
            uv_t.y = mod(uv_t.y, 1.0);

            vec2 lookup = uv_t*16.0;
            // vec2 lookup = uv_t*(length(fft)*2.0);
            // float noise = iqnoise(lookup, 1.0, u_random.x);
            // float noise = iqnoise(lookup, u_random.y, u_random.x);
            float noise = iqnoise(lookup, fft.x, fft.y);

            uv_s.x -= 0.25;
            uv_s.x += (1.0-(noise*2.0))*0.025;
    ...
    let player = await load_sound(`/assets/2019_09_27.mp3`)
