...
let sketch = async ({ gl, width, height }) => {
    ...
    let torus_render = regl({
        ...
        uniform sampler2D u_texture0, u_texture1, u_fft;
        ...
        void main() {
            ...
            vec3 color = smoothstep(tex1.rgb, tex0.rgb, vec3(st.y*t));
            // vec3 color = mix(tex1.rgb, tex0.rgb, t);

            gl_FragColor = vec4(color, tex0.a);
        }
        `,
        vert: `
    ...
    let torus_draw = regl({
        ...
        void main() {
            ...
            float t = u_time*PI;
            float noise1 = voronoi2d((uv.xy+u_random.xy)*2.0);
            float noise2 = voronoi2d((uv.xy+u_random.xz)*6.0);
            float noise3 = voronoi2d((uv.xy+u_random.yz)*8.0);

            float noise = noise1+noise2+noise3;

            position.xyz *= 1.0+(noise*norm.xyz*(length(fft)*0.125));
        ...
        uniforms: {
            ...
            u_matrix: (stats, props) => {
                let { u_time, u_random, u_resolution } = props
    ...
    let player = await load_sound(`/assets/2019_08_09.mp3`)
