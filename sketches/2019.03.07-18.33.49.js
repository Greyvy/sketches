...
let sketch = ({ gl, width, height }) => {
    ...
    let bgc = hsluv.hsluvToRgb([220*rand()*240, 80, 20+rand()*60])
    let tube_colours = tube.map((_) => {
        return hsluv.hsluvToRgb([rand()*240, 50+rand()*50, 25+rand()*50])
    })
        ...
        uniforms: {
            ...
            u_matrix: (_, props) => {
                ...
                let tra = mat4.translate([], mat4.identity([]), [
                    -8+(u_t*16),
                    u_fft[u_index]*0.025,
                    Math.sin(Math.cos(u_t+u_time*TAU)*TAU)
                ])
                let rot = mat4.rotate([], tra, (PI/2)+Math.sin(u_time*TAU)*PI/4, [
                    1,
                    u_random[1]*0.025,
                    u_random[2]*0.0125
                ])
                let mat = mat4.scale([], rot, [
                    u_fft[u_index]*0.25,
                    Math.cos(u_t-u_time*TAU),
                    0.125+(Math.sin(u_t+u_time*TAU)+1)
                ])
                return mat
            },
            u_t: regl.prop('u_t'),
    ...
    let player = new Tone.Player('/assets/2019_03_08.mp3').toMaster()
