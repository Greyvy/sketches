...
let sketch = async ({ gl, width, height }) => {
    ...
    let offscreen_render = regl({
        ...
        void main() {
            ...
            // vec4 color = vec4(0.98*texture2D(u_texture0, uv).rgb, 0.95);
            vec4 color = texture2D(u_texture0, uv);

            vec4 ft = fft*vec4(0.025);
            float r = texture2D(u_texture0, uv+vec2(ft.r)).r;
            float g = texture2D(u_texture0, uv-vec2(ft.g)).g;
            float b = texture2D(u_texture0, uv*vec2(ft.b)).b;
    ...
    let particle_draw = regl({
        ...
        uniforms: Object.assign({
            u_model: function(stats, {u_time, u_offset, u_scale, u_random}) {
                let result = mat4.create()
                result = mat4.rotate([], result, TAU*0.25, [1, 0, 1])
    ...
    let player = await load_sound(`/assets/2020_01_24.mp3`)
        ...
        render({ playhead, frame }) {
            ...
            render_target1024.use(function() {
                for (let i = 0; i < parts.length; ++i) {
                    let part = parts[i]
                    let pt = i/(parts.length-1)

                    let ft = fft_r[Math.floor(pt*fft_r.length)]*0.0125

                    part.u_offset[0] += part.u_velocity[0]*ft
                    part.u_offset[1] += part.u_velocity[1]*ft
                    part.u_offset[2] += part.u_velocity[2]*ft

                    // part.u_velocity[0] += (1.0-rand()*2.0)*0.0000125
                    // part.u_velocity[1] += (1.0-rand()*2.0)*0.0000125
                    part.u_velocity[2] += (1.0-rand()*2.0)*0.0000125

                    let d = Math.max(
                        1.0-(vec3.dist(vec3.fromValues(...part.u_offset), center)/1.5),
                        0.0)
                    let a = vec3.angle(center, vec3.fromValues(...part.u_offset))

                    let f = Math.pow(d, 0.125)
                    let vx = Math.cos(a)*f*0.0000125
                    let vy = Math.sin(a)*f*0.0000125

                    part.u_velocity[0] += vx
                    part.u_velocity[1] += vy
                    part.u_velocity[2] += (1.0-rand()*2.0)*ft*0.0000125

                    part.u_time = playhead
                    part.u_scale = 0.025+(d*0.25)
                    parts[i] = Object.assign(part, lights)

                    if (vec3.dist(vec3.fromValues(...part.u_offset), center) > 3.0) {
                        part.u_offset = [1.0-rand()*2.0, -2.0, 0]
                        part.u_velocity = [0, 0.0005+rand()*0.015, 0]
                    }
