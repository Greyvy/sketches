...
let sketch = ({ gl, width, height }) => {
    ...
    let cone_colours = cone.cells.map((v) => {
        let h = rand() > 0.5 ? 325 : 235
        return [
            hsluv.hsluvToRgb([h, 100, 50]),
            hsluv.hsluvToRgb([h, 100, 50]),
            hsluv.hsluvToRgb([h, 100, 50])
        ]
    })
    ...
    let cone_draw = regl({
        ...
        void main() {
            ...
            gl_FragColor = vec4(v_color*pow(depth, u_fft.x), u_fft.z);
        }`,
        ...
        uniforms: {
            u_view: ({time}, props) => {
                return mat4.lookAt([],
                    [0.0, 0.0, -4 ], // position of camera
                    [0.0, 0.0, 0.0], // point of view looking at
                    [0.0, 1.0, 0.0]  // pointing up
                )
            },
            u_matrix: (stats, props) => {
                let { u_fft, u_time, u_random } = props
                let sca = mat4.scale([], mat4.identity([]), [
                    1-u_fft[0],
                    1-u_fft[0],
                    u_fft[0]*4
                ])
                let tra = mat4.translate([], sca, [
    ...
    let player = new Tone.Player('/assets/2019_05_10.mp3').toMaster()
    let fft = new Tone.FFT(cone.positions.length)
    player.autostart = true
    player.loop = true
    player.connect(fft)

    let hue = rand() > 0.5 ? 325 : 235
    return ({ playhead }) => {
        let fft_v = fft.getValue()

        let bgc = hsluv.hsluvToRgb([
            hue,
            map(fft_v[0], 0, -100, 0, 100)*10,
            50
        ])
        regl.poll()
        ...
        cone_draw(cone_elements.map(function(v) {
            return Object.assign(v, {
                u_time: playhead,
                u_fft: [
                    map(fft_v[0], 0, -100, 0, 1),
                    map(fft_v[Math.floor(fft_v.length/2)], 0, -100, 0, 1),
                    map(fft_v[fft_v.length-1], 0, -100, 0, 1)
                ]
            })
        }))
