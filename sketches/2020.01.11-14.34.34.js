...
let sketch = async ({ gl, width, height }) => {
    ...
    let letters_draw = regl({
        ...
        uniforms: Object.assign({
            u_model: function(stats, {u_time, u_offset, u_t, u_scale}) {
                ...
                // result = mat4.rotate([], result, PI*0.5, [0, 1, 0])
    ...
    let words = new Array(16)
        ...
        .map(function(v, i, a) {
            ...
            return {
                u_scale: [0.365, 0],
                u_offset: [Math.cos(angle), Math.sin(angle), 0],
                u_t: i/(a.length-1)
            }
        })
        ...
        render({ playhead, frame }) {
            ...
            words.forEach(function(word) {
                letters_draw(Object.assign(word,
                    Object.assign({
                        u_time: playhead,
                        u_scale: spring(word.u_scale, 0.0125+rand()*0.365, 0.24, PI, 0.016)
                    }, lights)))
            })
