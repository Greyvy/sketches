...
let sketch = async ({ gl, width, height }) => {
    ...
    let offscreen_render = regl({
        ...
        void main() {
            ...
            vec4 color = vec4(0.98*texture2D(u_feedback, uv).rgb, 0.95);
            // color = texture2D(u_texture0, warp);

            float r = 0.98*texture2D(u_feedback, uv+vec2(0.000125)).r;
            float g = 0.98*texture2D(u_feedback, uv-vec2(0.000125)).g;
            float b = 0.98*texture2D(u_feedback, uv*vec2(0.000125)).b;
    ...
    let particle_draw = regl({
        ...
        void main() {
            ...
            mat2 rot = rotate2d(u_time+(a_fft*0.00125)*PI*2.0);

            // position.xy *= rot;
            position.xyz *= a_scale;
            position.xyz += ((1.0-a_fft*2.0)*0.25)+a_offset.xyz;
            position.xy *= rot;
        ...
        void main() {
            ...
            vec3 light = vec3(0.0);
            vec4 fft = texture2D(u_fft, v_vertex.xy);
    ...
    let player = await load_sound(`/assets/2020_02_14.mp3`)
    ...
    return {
        ...
        render({ playhead, frame }) {
            ...
            for (let i = 0; i < N_INSTANCES; ++i) {
                ...
                let vx = Math.cos(a)*f*0.00125
                let vy = Math.sin(a)*f*0.00125
                ...
                scales[i] = (2.0+((1.0-(vec3.dist(center, pos)/4.0))*4.0))*0.0025
            ...
            offscreen_render({
                ...
                // u_texture0: framebuffer_1024.color[0]
            })
