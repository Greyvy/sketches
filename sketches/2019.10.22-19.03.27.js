...
let sketch = async ({ gl, width, height }) => {
    ...
    let line_draw = regl({
        ...
        void main() {
            ...
            // position.xy -= (2.0-f*4.0)*fft.x;

            vec2 warp = mix(q, r, f);

            float line_width = u_linewidth+length(warp)*length(fft);
            vec3 p = vec3(position.xy+vec2(normal*line_width/2.0*mitre), u_fakez+(1.0-fft.z*2.0));

            gl_Position = u_projection*u_view*u_matrix*vec4(p, 1.0);

            v_edge = sign(mitre);
            v_uv = a_uv;
            v_vertex = u_projection*u_view*u_matrix*vec4(p, 1.0);
        }
        `,
        frag: `
        ...
        void main() {
            ...
            vec2 r = vec2(0.0);
            r.x = fbm(st+1.0*q+vec2(1.7,2.8)+0.126*t);
            r.y = fbm(st+1.0*q+vec2(8.3,9.8)+0.25*t);
        ...
        uniforms: {
            ...
            u_matrix: (stats, props) => {
                ...
                let rotate = mat4.rotate([], mat4.identity([]), u_time*TAU, u_random)
                let translate = mat4.translate([], rotate, [0, 0, Math.sin(u_time*TAU)*0.25])
    ...
    let player = await load_sound(`/assets/2019_10_25.mp3`)
