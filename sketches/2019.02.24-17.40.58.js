    ...
    */
    ...
    let draw_plane = regl({
        ...
        void main() {
            ...
            gl_FragColor = vec4(col, 1.0-(v_depth*0.75));
        }`,
        vert: `
        precision mediump float;
        ...
        uniforms: {
            ...
            u_matrix: (_, props) => {
                ...
                let tra = mat4.translate([], mat4.identity([]), [
                    Math.cos(u_t*TAU*rot_modifier)*((u_time*0.125)),
                    Math.sin(u_t*TAU*rot_modifier)*((u_time*0.125)),
                    2
                ])
                let sca = mat4.scale([], tra, [2, 2, 2])
    ...
    let draw_tube = regl({
        ...
        uniforms: {
            ...
            u_matrix: (_, props) => {
                ...
                let x = Math.cos(u_t*TAU)
                let y = Math.sin(u_t*TAU)
                let z = Math.sin(u_time*TAU)
                let tra = mat4.translate([], mat4.identity([]), [
                    x*u_offset[0]*6, y*u_offset[1]*6, z*u_offset[2]*4
                ])
                let sca = mat4.scale([], tra, [0.125, 0.125, 0.125])
    ...
    return ({ playhead }) => {
        ...
        draw_tube(tube_elements.map((v, i, a) => {
            ...
            let p = playhead
            let xo = Math.sin(t*TAU)
            let yo = Math.cos(t*t*TAU)
            let zo = Math.sin(t+32*TAU)
            ...
            return Object.assign(v, { u_time: playhead, u_offset: offset })
        }))
    }
}
