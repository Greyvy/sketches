...
let sketch = async ({ gl, width, height }) => {
    ...
    let offscreen_render = regl({
        ...
        void main() {
            ...
            vec4 fft = texture2D(u_fft, uv)*0.016;

            float n = iqnoise(uv, 1.0, 0.0);

            vec4 color = vec4(0.98*texture2D(u_feedback, warp+fft.rg).rgb, 0.95);
            // color = texture2D(u_texture0, warp);

            // float r = texture2D(u_feedback, uv+vec2(0.0125)).r;
            // float g = texture2D(u_feedback, uv-vec2(0.0125)).g;
            // float b = texture2D(u_feedback, uv*vec2(0.0125)).b;
    ...
    let particle_draw = regl({
        ...
        void main() {
            ...
            position.z += (1.0-a_fft*2.0);
            position.xy *= rot;

            gl_Position = u_projection*u_view*u_model*position;
            gl_PointSize = a_scale+(a_fft*2.0);
        ...
        void main() {
            ...
            color.rgb = texture2D(u_fft, v_vertex.xy).rgb;
        ...
        uniforms: Object.assign({
            u_model: function(stats, {u_time, u_random}) {
                ...
                result = mat4.rotate([], result, TAU*0.25, u_random)
    ...
    let player = await load_sound(`/assets/2020_02_07.mp3`)
    ...
    return {
        ...
        render({ playhead, frame }) {
            ...
            for (let i = 0; i < fft_v.length; ++i) {
                fft_r[i] = lerp(fft_r[i], Math.floor(map(fft_v[i], -40, 0, 0, 255)), 0.06)
            }
            ...
            render_target1024.use(function() {
                for (let i = 0; i < N_INSTANCES; ++i) {
                    ...
                    let d = Math.max(1.0-(vec3.dist(pos, center)/1.5), 0.0)
                    let a = Math.atan2(pos[1], pos[0])

                    let f = Math.pow(d, 8.0)
                    let vx = Math.cos(a)*f*0.000125
                    let vy = Math.sin(a)*f*0.000125

                    flow[i][0] = simplex.noise2D(playhead, ...offsets[i])
                    flow[i][1] = simplex.noise3D(playhead, ...offsets[i])
                    flow[i][2] = simplex.noise4D(playhead, ...offsets[i])

                    velocities[i][0] = vx
                    velocities[i][1] = vy
                    velocities[i][2] = flow[i][2]*0.00125

                    offsets[i][0] += velocities[i][0]
                    offsets[i][1] += velocities[i][1]
                    offsets[i][2] += velocities[i][2]

                     // scales[i] = 2.0+(vec3.dist(pos, center)/1.5)
                     // scales[i] = 4.0+(vec3.length(velocities[i])*8.0)
                     scales[i] = 2.0+((1.0-(vec3.dist(center, pos)/4.0))*4.0)
                }
