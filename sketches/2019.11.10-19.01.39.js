...
let sketch = async ({ gl, width, height }) => {
    ...
    let line_draw = regl({
        ...
        void main() {
            ...
            vec2 contrast = 1.0-u_spot.xy*2.0;
            float line_width = 0.0025+dot(contrast*0.025, position.xy);
        ...
        line_target1024.use(() => {
            for (let i = 0; i < lines_geo.length; ++i) {
                ...
                for (let j = 0; j <= N_POINTS; ++j) {
                    ...
                    let s = 2
                    let nx = simplex.noise3D(jt*s, it*s, Math.cos(it+playhead*TAU))
                    let ny = simplex.noise3D(jt*s, it*s, Math.sin(it+playhead*TAU))
