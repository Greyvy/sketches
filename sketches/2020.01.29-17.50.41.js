...
let sketch = async ({ gl, width, height }) => {
    ...
    let hue = rand()*360
    let sphere_colors = regl.buffer({
        data: Array(N_INSTANCES)
            .fill()
            .map(function() {
                return [...hsluv.hsluvToRgb([hue, 50+rand()*25, rand()*100]), 1.0]
            }),
        usage: 'dynamic'
    })
    ...
    let particle_draw = regl({
        ...
        uniforms: Object.assign({
            u_model: function(stats, {u_time, u_random}) {
                ...
                result = mat4.rotate([], result, TAU*0.25, [1, 1, 0])
    ...
    let player = await load_sound(`/assets/2020_01_31.mp3`)
    ...
    function pos_make() {
        return [(1.0-rand()*2.0)*0.5, 0.0, (1.0-rand()*2.0)*0.5]
    }

    function vel_make() {
        // return [0.0, 0.0005+rand()*0.015, 0.0]
        return [0.0, 0.0, 0.0]
    }
        ...
        render({ playhead, frame }) {
            ...
            render_target1024.use(function() {
                for (let i = 0; i < N_INSTANCES; ++i) {
                    ...
                    let fft = (1.0-(fft_r[i]/255)*2.0)

                    velocities[i][0] = simplex.noise2D(fft, offsets[i][0])*0.0125
                    velocities[i][0] = simplex.noise2D(fft, offsets[i][1])*0.0125
                    velocities[i][0] = simplex.noise2D(fft, offsets[i][2])*0.0125

                    velocities[i][0] += flow[i][0]*0.0000125
                    velocities[i][1] += flow[i][1]*0.0000125
                    velocities[i][2] += flow[i][2]*0.0000125

                    // velocities[i][0] += (1.0-rand()*2.0)*0.0000125
                    // velocities[i][1] += (1.0-rand()*2.0)*0.0000125
                    // velocities[i][2] += (1.0-rand()*2.0)*0.0000125
                    ...
                    scales[i] = 2.0+((vec3.dist(pos, center)/1.5)*4.0)

                    if (vec3.dist(pos, center) > 3.0) {
                        let p = pos_make()
                        let v = vel_make()
                        offsets[i][0] = p[0]
                        offsets[i][1] = p[1]
