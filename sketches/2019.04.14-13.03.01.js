...
let sketch = ({ gl, width, height }) => {
    ...
    let pane_colours = pane.positions.map((_, i) => {
        return hsluv.hsluvToRgb([0, 0, pane_colour])
    })
    ...
    let pane_draw_fbo = regl({
        ...
        void main() {
            ...
            gl_FragColor = vec4(u_t, u_t, u_t, (1.0-u_t)*0.5);
        }`,
        vert: `
        precision mediump float;
        ...
        uniforms: {
            ...
            u_matrix: (_, props) => {
                ...
                let rit = mat4.rotate([], tra, TAU*u_random[0], u_random)
                let mat = mat4.scale([], rit, [scale, scale, scale])
                return mat
            },
            u_t: regl.prop('u_t'),
    ...
    return ({ playhead }) => {
        ...
        viewport_draw(
            [{
                u_matrix: mat4.translate([], mat4.scale([], mat4.identity([]), [-1, -1, 1]), [-0.5, 0, 0]),
                u_texture: pane_1024fbo.color[0],
                u_random: [rand(), rand(), rand()]
            },
            {
                u_matrix: mat4.translate([], mat4.identity([]), [-0.5, 0, 0]),
