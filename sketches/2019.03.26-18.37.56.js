...
let sketch = ({ gl, width, height }) => {
    ...
    let ball_colours = ball.positions.map(() => {
        return hsluv.hsluvToRgb([0, 0, rand()*45])
    })
    ...
    let ball_draw = regl({
        ...
        uniforms: {
            ...
            u_matrix: (_, props) => {
                ...
                let scale = Math.pow(u_t, Math.sin(u_time*TAU)*0.125)*0.5
                let extense = 4+(Math.pow(u_t, Math.sin(u_time*TAU)*0.25)*Math.sin(u_time*TAU))*2

                let rot = mat4.rotate([],
                    mat4.identity([]), u_time*TAU, u_random)
                let tra = mat4.translate([], rot, [
                    map(u_index%u_stride, 0, u_stride, -extense, extense),
