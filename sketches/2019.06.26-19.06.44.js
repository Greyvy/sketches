...
let sketch = async ({ gl, width, height }) => {
    ...
    let text_make = function(str) {
        ...
        ctx.font = `${fs}px NationalTest-Extrabold`
    ...
    let box = box_make({ size: 2/(N+24) })

    let bgc = hsluv.hsluvToRgb([rand()*360, rand()*20, 80+rand()*20])

    let box_colour = hsluv.hsluvToRgb([rand()*360, 50+rand()*50, rand()*100])
    ...
    let box_draw = regl({
        ...
        void main() {
            ...
            vec4 fft = u_fftv;
            vec4 color = vec4(v_color, 0.75-depth);
        ...
        void main() {
            ...
            // position.x += map_range(u_fftv.x, 0.0, 255.0, -0.025, 0.025);
            position.y += map_range(u_fftv.y, 0.0, 255.0, -0.025, 0.025);
            position.z += map_range(u_fftv.z, 0.0, 255.0, -0.025, 0.025);
        ...
        uniforms: {
            ...
            u_matrix: (stats, props) => {
                ...
                */
                ...
                let x = u_position[0]
                let y = -u_position[1]
                let z = u_position[2] //*Math.sin(u_time*PI)*0.5

                return mat4.translate([], rot, [x, y, z])
            },
            u_projection: ({viewportWidth, viewportHeight}) => {
                return mat4.perspective([],
                    PI/2, viewportWidth/viewportHeight, 0.01, 1000)
            },
    ...
    let player = await load_sound(`/assets/2019_06_28.mp3`)
    ...
    return {
        ...
        render({ playhead }) {
            ...
            box_draw(box_elements.map(function(value, i) {
