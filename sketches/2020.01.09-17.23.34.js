...
let sketch = async ({ gl, width, height }) => {
    ...
    let letters_draw = regl({
        ...
        void main() {
            ...
            vec4 fft = texture2D(u_fft, vec2(0.0, abs(position.x)));
            float t = 1.0-pow(abs(1.0-u_time*2.0), 1.5);

            position.y = mix(position.y, position.y+(1.0-length(fft)*2.0)*0.5, 0.16);
            position.x += cos(length(fft)*PI*2.0)*0.125;
        ...
        uniforms: Object.assign({
            u_model: function(stats, {u_time, u_offset, u_z, u_scale}) {
                let t = Math.pow(Math.abs(1.0-u_time*2.0), 1.5)

                let y = t%0.5

                let result = mat4.create()
                result = mat4.rotate([], result, PI*-0.25, [1, 0, 0])
                result = mat4.rotate([], result, t*0.65, [0, 1, 1])
                result = mat4.translate([], result, [0, u_offset[1], u_z[0]])
    ...
    let player = await load_sound(`/assets/2020_01_10.mp3`)
    ...
    let words = new Array(16)
        ...
        .map(function(v, i, a) {
            return {
                u_z: [0, 0],
                u_scale: [0.45, 0],
                u_offset: [0, 4.0-(i/(a.length-1))*8.0, 0]
            }
        })
        ...
        render({ playhead, frame }) {
            ...
            words.forEach(function(word) {
                letters_draw(Object.assign(word,
                    Object.assign({
                        u_time: playhead,
                        u_z: spring(word.u_z, 1.0-rand()*2.0, 0.24, PI, 0.016),
                        u_scale: spring(word.u_scale, 0.025+rand()*0.65, 0.24, PI, 0.016)
                    }, lights)))
            })
