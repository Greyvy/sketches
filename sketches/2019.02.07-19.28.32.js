...
let sketch = ({ gl, width, height }) => {
    ...
    let cylindar_col = Array(cylindar_geo.length).fill([]).map(v => {
        return hsluv.hsluvToRgb([200+rand()*70, rand()*20, 20+rand()*40])
    })
    let cone_geo = cone(0.5, 2, 6)
    let cone_col = Array(cylindar_geo.length).fill([]).map(v => {
        return hsluv.hsluvToRgb([120+rand()*70, rand()*20, 20+rand()*40])
    })
    let bgc = hsluv.hsluvToRgb([rand()*360, 20+rand()*40, 80])
    ...
    let draw_bg = regl({
        ...
        void main() {
            vec3 col = u_color;
            col.x += sin(u_time*PI*2.0)*0.125;
            col.y += sin(u_time*PI*2.0)*0.125;
    ...
    let draw_cylindar = regl({
        ...
        void main() {
            // vec3 col = v_color;
            vec3 col = u_color;
            gl_FragColor = vec4(col, sin(u_time*PI));
        }`,
        vert: `
        precision mediump float;
    ...
    let draw_cone = regl({
        ...
        void main() {
            // vec3 col = v_color;
            vec3 col = u_color;
            gl_FragColor = vec4(col, 1.0-sin(u_time*PI));
        }`,
        vert: `
        precision mediump float;
        ...
        uniforms: {
            ...
            u_matrix: (_, props) => {
                ...
                let tra = mat4.translate([], mat4.identity([]), [
                    ...
                    -8+Math.sin(u_time*PI)*16,
                    Math.sin(u_t*TAU)*((st*12)+(u_offset[1]*1))
                ])
                let rot = mat4.rotate([], tra, u_t*PI, [1, 1, 0])
                let cyc = mat4.rotate([], rot, TAU, [0, 1, 0])
                let sca = mat4.scale([], cyc, [
                    Math.sin(u_time*PI),
                    Math.sin(u_time*PI),
                    Math.sin(u_time*PI)
                ])
                let mat = mat4.scale([], sca, u_offset)
                return mat
            },
            u_t: regl.prop('u_t'),
            u_time: regl.prop('u_time'),
