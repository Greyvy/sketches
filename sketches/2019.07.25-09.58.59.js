...
let sketch = async ({ gl, width, height }) => {
    ...
    let text_make = function(colour) {
        ...
        ctx.textAlign = 'left'
        ctx.fillText('totally', 8*scale, fs)
        ctx.fillText('uncontrollably', 8*scale, fs*2.0)
        ctx.fillText('sobbing', 8*scale, fs*3.0)
        ctx.fillText('sociopath', 8*scale, fs*4.0)
    ...
    let cube = cube_make(1, 1, 1, 16)
    let text = text_make()

    let bgc = hsluv.hsluvToRgb([rand()*360, rand()*20, 80+rand()*20])

    let cube_colour = hsluv.hsluvToRgb([rand()*360, 60+rand()*40, 50])
    ...
    let cube_draw = regl({
        ...
        void main() {
            ...
            float t = u_time*PI;
            float noise = voronoi2d((st+position.xy+u_random.xy)*1.5);
            vec3 noise3 = voronoi3d(position);

            // position *= noise3;
            // position.xyz *= 1.0+noise;

            position.z += sin(u_time*PI)*noise*fft.z*0.125;
            position.y += fft.y*0.125;
            position.xyz *= 1.0+length(fft.xyz*0.125);
        ...
        uniforms: {
            ...
            u_matrix: (stats, props) => {
                ...
                let rot = mat4.rotate([], tra, u_time*PI*2.0, [u_random[0], u_random[1], 0])
                let sca = 0.75+Math.sin(map(u_index, 0, 2, 0, 1)*PI)*0.25
                return mat4.scale([], rot, [sca, -sca, sca])
            },
    ...
    let player = await load_sound(`/assets/2019_07_29.mp3`)
    ...
    return {
        ...
        render({ playhead }) {
            ...
            for (let i = 0; i < fft_v.length; ++i) {
                fft_r[i] = Math.floor(map(fft_v[i], -10, 0, 0, 255))
            }
