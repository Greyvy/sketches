...
let sketch = async ({ gl, width, height }) => {
    ...
    let bgc = hsluv.hsluvToRgb([rand()*360, rand()*20, 75+rand()*25])

    let icosphere_colour = hsluv.hsluvToRgb([rand()*360, 75+rand()*25, 25+rand()*50])
    ...
    let player = await load_sound(`/assets/2019_07_12.mp3`)
    ...
    return {
        ...
        render ({ playhead }) {
            ...
            for (let i = 0; i < fft_v.length; i++) {
                fft_r[i] = Math.floor(map(fft_v[i], -120, 0, 0, 255))
            }
