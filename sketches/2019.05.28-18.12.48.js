...
let sketch = ({ gl, width, height }) => {
    ...
    let bgc = hsluv.hsluvToRgb([0, 0, rand()*10])
    ...
    let plane_draw = regl({
        ...
        void main() {
            ...
            vec2 uv = v_uv;
            // uv.x += sin(st.x*PI)*(u_random.x+u_random.y)+u_time;
            // uv.y += sin(u_time*st.y*PI*2.0);
            ...
            color.a *= clamp(clamp(pow(u_time*8.0, 8.0), 0.0, 1.0)-pow(clamp(length(noise.rgb), 0.0, 1.0), 4.0), 0.0, 1.0);
