...
let sketch = ({ gl, width, height }) => {
    ...
    let bgc = hsluv.hsluvToRgb([rand()*360, 50, rand()*100])
    let ball_colour = hsluv.hsluvToRgb([rand()*360, 50, rand()*100])
    let ball_colours = ball.positions.map(() => {
        return ball_colour //hsluv.hsluvToRgb([rand()*360, 50, rand()*100])
    })
    ...
    let ball_draw = regl({
        ...
        uniforms: {
            ...
            u_matrix: (_, props) => {
                ...
                //let scale = Math.pow(u_t, Math.sin(u_time*TAU)*0.125)*0.5*(u_fft[31]*0.002)
                let scale = u_fft[31]*0.0005
                //let extense = 4+(Math.pow(u_t, Math.sin(u_time*TAU)*0.25)*Math.sin(u_time*TAU))*2
                let extense = u_fft[4]*0.05

                let rot = mat4.rotate([],
                    mat4.identity([]), u_time*TAU, [u_random[0], 0, 1])
                let tra = mat4.translate([], rot, [
                    map(u_index%u_stride, 0, u_stride, -extense-(u_fft[16]*0.05), extense+(u_fft[16]*0.05)),
                    map(Math.floor(u_index/u_stride)%u_stride, 0, u_stride, -extense-(u_fft[0]*0.05), extense+(u_fft[0]*0.05)),
                    map(Math.floor(u_index/(u_stride*u_stride)), 0, u_stride, -extense, extense)
                ])
    ...
    let ball_elements = Array(16*16*16)
        .fill({})
        .map((_, i, a) => {
            return {
                u_random: [
    ...
    let player = new Tone.Player('/assets/2019_03_29.mp3').toMaster()
