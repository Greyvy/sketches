...
let sketch = ({ gl, width, height }) => {
    ...
    let ball_colours = ball.positions.map(() => {
        return hsluv.hsluvToRgb([rand()*360, rand()*100, rand()*100])
    })
    ...
    let ball_draw_fbo = regl({
        ...
        uniforms: {
            ...
            u_matrix: (_, props) => {
                ...
                let scale = 4+(u_fft[16]*0.025)

                let tra = mat4.translate([], mat4.identity([]), [
                    Math.sin(u_t*TAU)*(4+(u_fft[0]*0.05)),
                    Math.cos(u_t*TAU)*(Math.sin(u_t*TAU)*(4+(u_fft[31]*0.05))),
                    0
                ])
                let rot = mat4.rotate([], tra, u_time*TAU, [ 1, 0, 1 ])
    ...
    let player = new Tone.Player('/assets/2019_04_05.mp3').toMaster()
    ...
    return ({ playhead }) => {
        ...
        regl.clear({ color: bgc, depth: 1, framebuffer: ball_fbo })
