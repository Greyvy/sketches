...
let sketch = async ({ gl, width, height }) => {
    ...
    let lines_render = regl({
        ...
        void main() {
            ...
            vec4 color = mix(base, vec4(0.0), circle.r);
    ...
    let line_draw = regl({
        ...
        void main() {
            ...
            mat2 r = rotate2d(length(fft)*0.25+u_spot.x*PI);
            float n = iqnoise(st+vec2(u_time, u_random.y), 1.0, u_random.x);

            float dist = distance(position.xy, 1.0-u_spot.xy+(length(fft)*0.125)*2.0);
        ...
        void main() {
            ...
            color = mix(color, fft, st.x);

            float inner = length(fft)*sin((PI/2.0)+st.y*PI);
        ...
        uniforms: {
            ...
            u_matrix: (stats, props) => {
                ...
                return mat4.rotate([], translate, 0, [1, 0, 0])
            },
    ...
    let player = await load_sound(`/assets/2019_11_01.mp3`)
