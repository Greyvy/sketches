...
let sketch = async ({ gl, width, height }) => {
    ...
    let text_make = function(colour) {
        ...
        ctx.textAlign = 'left'
        ctx.fillText('part', 8*scale, fs*2.0)
    ...
    let icosphere = icosphere_make(1.0, { subdivisions: 1 })
    let text = text_make()

    let bgc = hsluv.hsluvToRgb([rand()*180, 100, rand()*25])

    let icosphere_colour = hsluv.hsluvToRgb([180+rand()*180, 100, 50+rand()*25])
    ...
    let icosphere_target256 = regl.framebuffer({
        color: [ regl.texture({ type: 'float', width: 256, height: 256 }) ]
    })
    let icosphere_target128 = regl.framebuffer({
    ...
    let icosphere_render = regl({
        ...
        void main() {
            ...
            vec4 color = mix(lowr, base, base.a);
    ...
    let icosphere_draw = regl({
        ...
        void main() {
            ...
            uv_t.y += pow(u_time, noise*0.0125);
        ...
        uniform mat4 u_projection, u_view, u_matrix;
        uniform vec3 u_random, u_translate;
        uniform vec2 u_resolution, u_beat;
        uniform float u_time, u_index, u_length, u_scale;
        ...
        void main() {
            ...
            vec4 beat = texture2D(u_fft, u_beat);
            ...
            float angle = length(beat)+u_time*PI*(2.0+floor(u_random.x*4.0));
            mat4 rotate = rotate3d(1.0+u_random*4.0, angle);
            vec4 point = vec4(normalize((u_random*2.0)-1.0)*u_translate, 1.0);
            vec4 scale = vec4(u_scale+(pow(u_scale, length(beat))));
            ...
            v_depth = position.z;
            v_vertex = (u_matrix*position).xyz;

            gl_Position = u_projection*u_view*u_matrix*position;
        }`,
        ...
        uniforms: {
            ...
            u_projection: ({viewportWidth, viewportHeight}) => {
                return mat4.perspective([],
                    // @NOTE(Grey): Some cheetz to hide some of the overlap in
                    // the back with the center sphere, lazy
                    PI/2, viewportWidth/viewportHeight, 0.01, 1.85)
            },
            u_time: regl.prop('u_time'),
            ...
            u_translate: regl.prop('u_translate'),
            u_fft: regl.prop('u_fft'),
            u_beat: regl.prop('u_beat')
        },
        blend: {
            enable: true,
            func: {
    ...
    let player = await load_sound(`/assets/2019_09_13.mp3`)
    ...
    return {
        ...
        render({ playhead }) {
            ...
            regl.clear({ color: bgc, depth: 1, framebuffer: icosphere_target256 })
            regl.clear({ color: bgc, depth: 1, framebuffer: icosphere_target128 })
            regl.clear({ color: bgc, depth: 1, framebuffer: icosphere_target64 })

            let fft_v = fft.getValue()
            for (let i = 0; i < fft_v.length; ++i) {
                fft_r[i] = Math.floor(map(fft_v[i], -130, 0, 0, 255))
            }
            icosphere_fft.subimage(fft_r)

            icosphere_target1024.use(() => {
                icosphere_draw(icosphere_elements.map(function(value, i, a) {
                    return Object.assign(value, {
                        u_time: playhead,
                        u_fft: icosphere_fft,
                        u_beat: [(i%(N/4))/(N/4), (Math.floor(i/(a.length-1)))/(N/4)]
                    })
                }))
            })

            icosphere_target128.use(() => {
                icosphere_draw({
                    u_random: icosphere_elements[0].u_random,
                    u_resolution: [width, height],
                    u_text: text,
                    u_index: 0,
                    u_length: 1,
            ...
            icosphere_render({
                ...
                u_texture1: icosphere_target128.color[0],
                u_scale: [2, 2, 2],
                u_time: playhead,
                u_snap: rand(),
                u_fft: icosphere_fft
