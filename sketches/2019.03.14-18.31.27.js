...
let sketch = ({ gl, width, height }) => {
    ...
    let start_hue = 40+rand()*320
    let end_hue = 60+rand()*300
    let bgc = hsluv.hsluvToRgb([start_hue, 50, 25+rand()*50])
    let cone_colours = cone.map((_, i, a) => {
        return hsluv.hsluvToRgb([lerp(start_hue, end_hue, i/(a.length-1)), rand()*50, 25+rand()*50])
    })
    ...
    let cone_draw = regl({
        ...
        uniforms: {
            ...
            u_matrix: (_, props) => {
                let { u_t, u_fft, u_index, u_stride, u_random, u_time } = props
                ...
                */
                ...
                let rot = mat4.rotate([], mat4.identity([]), u_t*TAU,
                    [
                        1, 1, 0
                    ])
                let tra = mat4.translate([], rot, [
                    2+u_fft[u_index]*0.05,
                    2+u_fft[u_index]*0.05,
                    -4+(Math.sin(u_random[1]+u_time*TAU)*0.35)
                ])
                let mat = mat4.scale([], tra, [ 0.25, 0.25, 1, ])
                return mat
            },
            u_t: regl.prop('u_t'),
    ...
    let player = new Tone.Player('/assets/2019_03_15.mp3').toMaster()
