...
vec4 color = base;
    ...
    function line_make(points) {
        ...
        for (let i = 2; i < N; i += 2) {
            ...
            if (i === 2) {
                // positions[i+0] = positions[i+2] = prev[0]
                // positions[i+1] = positions[i+3] = prev[1]
            ...
            if (!next) {
                // positions[i+0] = positions[i+2] = curr[0]
                // positions[i+1] = positions[i+3] = curr[1]
    ...
    for (let i = 0; i < N_LINES; ++i) {
        ...
        for (let j = 0; j <= N_POINTS; ++j) {
            let jt = j/N_POINTS
            let rt = Math.sin(jt*TAU)
            let x = simplex.noise2D(i+Math.pow(jt, 1.5), i+2.0-jt*4.0)*1.0
            let y = 2.0-jt*4.0
            // let r = 0.5+simplex.noise2D(i+Math.pow(rt, 1.5), i+2.0-rt*4.0)*0.125
    ...
    let line_draw = regl({
        ...
        void main() {
            ...
            float line_width = 0.0125+velocity.x*6.0;
            // float line_width = noise*0.125;
    ...
    return ({ playhead, frame }) => {
        ...
        for (let i = 0; i < lines_geo.length; ++i) {
            ...
            // line.positions[0] = line.positions[2] = lerp(line.positions[0], random_wags[i][0], 0.06)
            // line.positions[1] = line.positions[3] = lerp(line.positions[1], random_wags[i][1], 0.06)
            for (let i = 4; i < line.positions.length; i += 4) {
                ...
                let velx = line.positions[i+0]-lerp(cx0, px0, 0.06)

                line.velocities[i+0] = line.velocities[i+2] = velx
                line.positions[i+0] = line.positions[i+2] = lerp(cx0, px0, 0.06)
                // line.positions[i+1] = line.positions[i+3] = lerp(cy0, py0, 0.06)
            }
        }
