...
let sketch = ({ gl, width, height }) => {
    ...
    let torus_draw = regl({
        ...
        void main() {
            ...
            uv_t.x -= u_time+(noise*0.25);
            uv_t.y += sin(u_time*PI)*0.025;

            vec4 text = texture2D(u_text, uv_t);
            vec4 color = vec4(v_color, 1.0);

            color.rgb = mix(color.rgb, vec3(1.0-v_color), text.r);
            ...
            color.rgb = mix(color.rgb, vec3(1.0-color.rgb), edge);
        ...
        void main() {
            ...
            float offset = voronoi2d(vec2(u_index/u_length, ease(sin(u_time*PI), 0.5))*2.0);
