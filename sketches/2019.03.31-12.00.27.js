...
let sketch = ({ gl, width, height }) => {
    ...
    let ball = icosphere(2)

    let bgc = hsluv.hsluvToRgb([0, 0, rand()*50])
    let ball_colour = hsluv.hsluvToRgb([0, 0, 50+rand()*45])
    ...
    let ball_draw = regl({
        ...
        uniforms: {
            ...
            u_matrix: (_, props) => {
                ...
                let scale = Math.sin(Math.sin(u_time*PI*4)+u_t*TAU)

                let tra = mat4.translate([], mat4.identity([]), [
                    map(Math.sin(u_time*TAU)+u_t, 0, 2, 8, -16),
                    Math.cos(u_t*TAU*16)*(Math.sin(u_t*TAU)*4),
                    Math.sin(u_t*TAU*8)
                ])
                let rot = mat4.rotate([], tra, u_time*TAU, [ 1, 1, 0 ])
