...
let sketch = async ({ gl, width, height }) => {
    ...
    let cube_draw = regl({
        ...
        uniforms: {
            ...
            u_matrix: (stats, props) => {
                ...
                let t = ease(Math.sin(u_time*PI), 1.5)
                let rt = vec3.mul([], u_random, [0.025+t, 0.025+t, 0.025+t])
    ...
    let player = await load_sound(`/assets/2019_10_04.mp3`)
    ...
    let cube_elements = new Array(128/4)
        .fill({})
        .map(function(value, i, a) {
            return {
                ...
                u_length: a.length,
                u_scale: 0.25,
                u_color: hsluv.hsluvToRgb([rand()*360, rand()*100, rand()*100]),
                u_lookup: [(i%(128/4))/(128/4), Math.floor(i/(128/4))/(128/4)]
            }
        })
