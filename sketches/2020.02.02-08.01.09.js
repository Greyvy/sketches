        ...
        render({ playhead, frame }) {
            ...
            render_target1024.use(function() {
                for (let i = 0; i < N_INSTANCES; ++i) {
                    ...
                    flow[i][0] = simplex.noise2D(playhead, offsets[i][0])
                    flow[i][1] = simplex.noise3D(playhead, offsets[i][0], offsets[i][1])
                    flow[i][2] = simplex.noise4D(playhead, offsets[i][0], offsets[i][1], offsets[i][2])

                    velocities[i][0] = vx + flow[i][0]*0.00125
                    velocities[i][1] = vy + flow[i][1]*0.00125
                    ...
                    //     velocities[i][2] = v[2]
                     ...
                     scales[i] = 2.0+(vec3.length(velocities[i])*4.0)
                }
