...
let sketch = async ({ gl, width, height }) => {
    ...
    let plane_colour = hsluv.hsluvToRgb([rand()*360, rand()*100, rand()*100])
    ...
    let plane_draw = regl({
        ...
        void main() {
            ...
            text.rgb = text.r < 1.0 ? fft.grr : vec3(1.0);
            // text.rgb *= fft.gbr;
        ...
        void main() {
            ...
            // position.xyz *= noise3;
            position.z -= wrap(quadratic_in_out(u_time))*(noise*(length(fft)*0.25));
    ...
    let player = await load_sound(`/assets/2019_07_19.mp3`)
